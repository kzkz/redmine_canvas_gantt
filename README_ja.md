...
**🌙 このフォークはダークモードと 🎌 日本の祝日表示を追加しています** — [このフォークの追加機能 ↓](#このフォークの追加機能)
...
**[tiohsa/redmine_canvas_gantt](https://github.com/tiohsa/redmine_canvas_gantt)** のフォークで、以下を追加しています。その他は本家と同じです。

### 🌙 ダークモード

- ツールバーに **ライト / ダーク / 自動** の切替ボタンを追加。
- **自動**は周囲の Redmine テーマに追従(ページ背景から判定)するため、ダークテーマなら設定不要でダーク表示になります。
- 選んだ設定は**ユーザー単位で全プロジェクトに適用**されます。
- キャンバス本体・ツールバー・メニュー・ダイアログ・ヘルプ画面・入力欄まで対応。

### 🎌 日本の祝日表示

- 祝日を土日と同様にチャート/見出しへ網掛け表示(**振替休日**・**国民の休日**を含め、外部サービスやデータファイル不要でオフライン計算)。
- 見出しの祝日に**マウスを乗せると祝日名**を表示。
- 日番号の色は日本のカレンダー準拠(日曜・祝日=赤、土曜=青)。
